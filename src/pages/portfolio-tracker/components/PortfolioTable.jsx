...
function SourceDot({ source }) {
  const color =
    source === 'live' ? 'bg-emerald-500' : source === 'manual' ? 'bg-amber-500' : 'bg-slate-600'
  ...
  return (
    ...
    >
      <input
        ...
        className="w-24 rounded-md border border-slate-700 bg-slate-950/60 px-2 py-1 text-xs text-slate-100 focus:border-teal-500 focus:outline-none"
      />
...
export default function PortfolioTable({ positions, livePrices, onEdit, onDelete, onSetManualPrice }) {
  ...
  if (positions.length === 0) {
    return (
      <Card className="border-dashed p-10 text-center text-sm text-slate-500">
          ...
          <tr>
            {COLUMNS.map((col) => (
              <th
                ...
                className="cursor-pointer select-none whitespace-nowrap px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-400 hover:text-slate-200"
              >
                {col.label}
          ...
          {rows.map(({ position, metrics }) => (
            <tr key={position.id} className="hover:bg-slate-800/40">
              <td className="whitespace-nowrap px-4 py-3 font-medium text-slate-100">
                {position.name}
                <span className="ml-1.5 text-xs font-normal text-slate-500">{position.ticker}</span>
              </td>
              <td className="whitespace-nowrap px-4 py-3 text-slate-300">{TYPE_LABELS[position.type]}</td>
              <td className="whitespace-nowrap px-4 py-3 text-slate-300">{position.quantity}</td>
              <td className="whitespace-nowrap px-4 py-3 text-slate-300">{formatCurrency(position.buyPrice)}</td>
              ...
              <td className="whitespace-nowrap px-4 py-3">
                <div className={metrics.gainPct >= 0 ? 'text-emerald-400' : 'text-rose-400'}>
              ...
              <td className="whitespace-nowrap px-4 py-3">
                <div className="flex gap-2">
                  <button
                    ...
                    className="text-xs font-medium text-slate-400 hover:text-teal-300"
                  >
                    Modifier
                  </button>
                  <button
